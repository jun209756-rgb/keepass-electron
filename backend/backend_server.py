...
def handle(request_id: str, command: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if command == "generate":
        ...
        def progress(done: int, total: int, message: str) -> None:
            send({"id": request_id, "type": "progress", "done": done, "total": total, "message": message})
        ...
        try:
            ...
            try:
                ...
                print(f"[warning] KDBX generated but log failed: {log_error}", file=sys.stderr, flush=True)
            return {"files": [str(path) for path in files], "entryCount": result.entry_count, "logsWritten": logs_written}
...
def main() -> None:
    ...
    try:
        ...
        interactive = False
    if interactive:
        print("KeePass Studio backend component.")
        print("Please launch KeePassStudio.exe instead of running keepass_backend.exe directly.")
        try:
